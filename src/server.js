...
app.post("/jobs", async (req, reply) => {
  ...
  try {
    ...
    const includedFiles = payload.files.filter((file) => !file.excluded);
    const missingFiles = includedFiles.filter((file) => !fileMap.has(file.id));
    if (includedFiles.length === 0) {
      reply.code(400);
      return { error: "no included files uploaded" };
    }
    if (missingFiles.length > 0) {
      reply.code(400);
      return { error: `missing uploaded file data for: ${missingFiles.map((file) => file.name ?? file.id).join(", ")}` };
    }
...
function parsePayload(value) {
  ...
  try {
    ...
    throw Object.assign(new Error("invalid JSON payload field"), { statusCode: 400 });
  }
}
